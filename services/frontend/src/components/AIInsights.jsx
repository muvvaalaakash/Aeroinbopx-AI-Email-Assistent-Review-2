  ...
  if (isLoading) {
    return (
      <div className="flex flex-col items-center justify-center h-full p-8 space-y-4">
        ...
        <p className="text-xs text-slate-500 dark:text-slate-400 font-medium">AI is generating summaries & drafts...</p>
      </div>
    );
  }
  ...
  if (!insights) {
    return (
      <div className="flex flex-col items-center justify-center h-full p-8 text-center space-y-3">
        ...
        <p className="text-xs font-semibold text-slate-700 dark:text-slate-300">Select an email to view AI Insights</p>
  ...
  return (
    <div className="h-full flex flex-col p-6 space-y-5 overflow-y-auto border-l border-slate-200 dark:border-slate-800/60 bg-slate-50/50 dark:bg-[#090d16]/40 transition-all duration-150">
      ...
      <div className="border-b border-slate-200 dark:border-slate-800/60 pb-3 flex items-center space-x-2">
        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor" className="w-4 h-4 text-indigo-500 dark:text-indigo-400">
          <path strokeLinecap="round" strokeLinejoin="round" d="M3.75 13.5l10.5-11.25L12 10.5h8.25L9.75 21.75 12 13.5H3.75z" />
        </svg>
        <h3 className="text-[11px] font-bold uppercase tracking-wider text-slate-500 dark:text-slate-400">AI Co-Pilot Insights</h3>
      </div>
      ...
      {folder === 'SPAM' && is_spam_false_positive && (
        <div className="p-3.5 rounded-lg border border-amber-500/25 bg-amber-500/5 text-xs text-amber-800 dark:text-amber-400 leading-relaxed font-semibold">
          ...
          <strong>Spam False-Positive:</strong> Gemini has flagged this as an important legitimate message!
          {spam_analysis_reason && (
            <p className="mt-1 font-medium text-slate-600 dark:text-slate-300 text-[11px]">Reason: {spam_analysis_reason}</p>
          )}
        </div>
      )}
      ...
      {is_meeting_request && (
        <div className="p-3 rounded-lg border border-blue-500/20 bg-blue-500/5 text-xs text-blue-800 dark:text-blue-400 leading-relaxed font-semibold flex items-center space-x-2">
          <span>📅</span>
          <span><strong>Meeting Invitation:</strong> Sender is requesting a call or schedule.</span>
        </div>
      )}
      ...
      {has_deadline && (
        <div className="p-3 rounded-lg border border-rose-500/20 bg-rose-500/5 text-xs text-rose-800 dark:text-rose-400 leading-relaxed font-semibold flex items-center space-x-2">
          <span>⏳</span>
          <span>
            <strong>Deadline Alert:</strong> Action required {deadline_date ? `by: ${deadline_date}` : 'promptly'}.
          </span>
        </div>
      )}
      ...
      <div className="space-y-1.5">
        <span className="text-[10px] font-bold text-slate-400 dark:text-slate-500 uppercase tracking-wider">Priority Level</span>
        <div>
          <span
      ...
      {/* Summary */}
      <div className="space-y-1.5">
        <span className="text-[10px] font-bold text-slate-400 dark:text-slate-500 uppercase tracking-wider">Executive Summary</span>
      ...
      <div className="space-y-3 flex-1 flex flex-col justify-between">
        <div className="space-y-1.5 flex-1 flex flex-col">
          <span className="text-[10px] font-bold text-slate-400 dark:text-slate-500 uppercase tracking-wider">Suggested Reply</span>
