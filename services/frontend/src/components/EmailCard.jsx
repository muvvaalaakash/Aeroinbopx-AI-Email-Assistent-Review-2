...
export default function EmailCard({ email, isSelected, onClick, aiInsights }) {
  ...
  // AI flags
  const isMeeting = email.ai_analysis?.is_meeting_request || aiInsights?.is_meeting_request;
  const hasDeadline = email.ai_analysis?.has_deadline || aiInsights?.has_deadline;
  const deadlineDate = email.ai_analysis?.deadline_date || aiInsights?.deadline_date;
  ...
  return (
    ...
    >
      ...
      <div className={`text-xs truncate mb-1 ${
        isUnread ? 'font-bold text-slate-700 dark:text-slate-300' : 'text-slate-500 dark:text-slate-400'
      }`}>
        {email.subject}
      </div>
      ...
      <div className="mt-2.5 flex items-center justify-between">
        ...
        <div className="flex items-center space-x-1.5">
          ...
          {isMeeting && (
            <span className="inline-flex items-center px-1.5 py-0.5 rounded bg-blue-500/10 dark:bg-blue-500/10 border border-blue-500/20 text-[9px] font-bold text-blue-600 dark:text-blue-400 shadow-sm" title="Meeting Request">
              📅 Call
            </span>
          )}
          ...
          {hasDeadline && (
            <span className="inline-flex items-center px-1.5 py-0.5 rounded bg-rose-500/10 dark:bg-rose-500/10 border border-rose-500/20 text-[9px] font-bold text-rose-600 dark:text-rose-400 shadow-sm" title={deadlineDate ? `Deadline: ${deadlineDate}` : 'Deadline warning'}>
