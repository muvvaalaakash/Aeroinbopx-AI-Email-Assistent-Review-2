...
class EmailAnalysis(BaseModel):
    ...
    is_spam_false_positive: bool = Field(default=False, description="True if the email was in spam but is actually a legitimate business email, a client inquiry, or meeting invitation (false positive spam).")
    spam_analysis_reason: str = Field(default="", description="If the email is a spam false positive, give a brief 1-sentence reason (e.g. 'Legitimate business query from client').")
    is_meeting_request: bool = Field(default=False, description="True if this is a calendar invite, meeting request, call scheduler, or request to meet.")
    has_deadline: bool = Field(default=False, description="True if a task deadline or urgent date is mentioned in the email.")
    deadline_date: str = Field(default="", description="The specific deadline date/time or timeframe (e.g. 'Friday at noon', 'June 10') if present.")
